...
def inchi_smiles_completion(CONCATENATE_LIST):
    ...
    for spectrum in tqdm(CONCATENATE_LIST, total=len(CONCATENATE_LIST), unit=" spectrums", colour="green", desc="{:>80}".format("processing")):
    ...
    for spectrum in tqdm(CONCATENATE_LIST, total=len(CONCATENATE_LIST), unit=" spectrums", colour="green", desc="{:>80}".format("updating")):
        if re.search("INCHIKEY: (.*)\n", spectrum):
            inchikey = re.search("INCHIKEY: (.*)\n", spectrum).group(1)
        # INCHI
        if re.search("\nINCHI: (.*)\n", spectrum):
...
def remove_no_smiles_inchi(CONCATENATE_LIST):
    ...
    for spectrums in tqdm(CONCATENATE_LIST, total=len(CONCATENATE_LIST), unit=" spectrums", colour="green", desc="{:>80}".format("processing")):
    ...
    for files in os.listdir(INPUT_path):
        if files.endswith(".msp"):
            ...
            if not re.search("FRAGHUBID: (.*)\n",content):
                ...
                for spectrums in tqdm(content, total=len(content), unit=" spectrums", colour="green", desc="{:>80}".format("processing")):
...
def mols_derivator(CONCATENATE_DF):
    ...
    t = tqdm(total=total_rows, unit=" rows", colour="green", desc="{:>80}".format("generating"))
...
def mass_calculator(CONCATENATE_DF):
    ...
    """
    total_rows = len(CONCATENATE_DF)
    t = tqdm(total=total_rows, unit=" rows", colour="green", desc="{:>80}".format("processing"))
