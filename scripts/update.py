...
def init_json_update_file(json_update_file):
    ...
    :return: The initialized JSON update file. If the input JSON update file is None, a new dictionary will be created with the key "FRAGHUBID_LIST" and an empty list as its value. If the
    * input JSON update file is not None, it will be returned as is.
    ...
    # and initialize a new dictionary with "FRAGHUBID_LIST" key and an empty value,
...
def check_for_update(spectrum):
    ...
    """
    # Extract the FRAGHUBID from the provided spectrum
    fraghub_id_spectrum = spectrum["FRAGHUBID"]
    # Get the dictionary of FRAGHUBID from the json file
    ...
    if not fraghub_id_dict or fraghub_id_spectrum not in fraghub_id_dict:
        # If the dictionary is empty, or the FRAGHUBID is not present in the dictionary, an update is needed. Return the spectrum and the FRAGHUBID
        return spectrum, fraghub_id_spectrum
    else:
        # If the FRAGHUBID is found in the dictionary, an update is not needed. Return None
        return None
...
def check_for_update_processing(spectrum_list, profile_name):
    ...
    # Updating json file with new Fraghubid list
