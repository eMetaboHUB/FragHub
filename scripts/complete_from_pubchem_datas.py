...
def complete_from_pubchem_datas(CONCATENATE_DF, progress_callback=None, total_items_callback=None, prefix_callback=None, item_type_callback=None):
    """
    ...
    """
    # Define the prefix for the task and the type of items if provided
    if prefix_callback:
    ...
    # Set the total number of items for tracking (rows in CONCATENATE_DF)
    if total_items_callback:
        total_items_callback(len(concatenate_df_copy), 0)  # Total to process, initially 0 completed

    # Enrich the DataFrame by merging it with pubchem_datas
    ...
    for col in columns_to_update:
        # Update cells in the columns with missing data
