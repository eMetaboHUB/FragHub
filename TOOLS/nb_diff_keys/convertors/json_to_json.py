...
def json_to_json(json_dict):
    ...
    This method takes a JSON object represented as a dictionary and checks if it contains the keys "compound", "id",
    "metaData", and "spectrum". If all of these keys are present, the method
    * calls the function "convert_MoNA_json" to convert the JSON object. Otherwise, it returns the original JSON object.

    Note: The "convert_MoNA_json" function is not implemented in this code, so you need to define it separately before
    using this method.
    """
    ...
    if all(key in json_dict for key in keys_to_check):
        ...
        return json_dict.keys()  # return dictionary keys here
    else:
